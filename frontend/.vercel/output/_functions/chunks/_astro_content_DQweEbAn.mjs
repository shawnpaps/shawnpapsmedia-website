import { f as renderElement, g as generateCspDigest, s as spreadAttributes, u as unescapeHTML, b as renderTemplate, h as removeBase, i as isRemotePath, j as unflatten, A as AstroError, U as UnknownContentCollectionError, o as object, k as date, l as array, R as RenderUndefinedEntryError, n as escape, p as string, q as prependForwardSlash, t as createHeadAndContent, r as renderComponent } from './entrypoint_DU0NaWmc.mjs';
...
class ImmutableDataStore {
  ...
  static async fromModule() {
    try {
      const data = await import('./_astro_data-layer-content_yNwcbA3l.mjs');
...
function createGetCollection({
  ...
  return async function getCollection(collection, filter) {
    ...
    if (store.hasCollection(collection)) {
      const { default: imageAssetMap } = await import('./content-assets_CvitAEEe.mjs');
      const result = [];
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import('./_astro_assets_C48kyfHy.mjs').then(n => n._);
...
async function renderEntry(entry) {
  ...
  if (entry.deferredRender) {
    try {
      const { default: contentModules } = await import('./content-modules_B8Q_q-65.mjs');
